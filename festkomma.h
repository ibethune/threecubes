...
#define addf_ssaaaa(sh, sl, ah, al, bh, bl)                                    \
    __asm__("addq %5,%1\n\tadcq %3,%0"                                         \
            : "=r"(sh), "=&r"(sl)                                              \
            : "0"((UDItype)(ah)), "g"((UDItype)(bh)), "%1"((UDItype)(al)),     \
              "g"((UDItype)(bl)))
...
#define mpxg_get_d_simple(fixed)                                               \
    (ldexp(((double)(fixed)[0]), -64) + ((double)((signed long)(fixed)[1])))
...
inline double mpxg_get_d(mpx_t fixed)
{
    ...
    if ((signed long)fixed[0] < 0)
    {
        /* Kuenstlich komplizierte Umrechnung in double.
           Der erste Summand ist [erg] - 1, was eventuell betragsmaeszig klein
           ist
    ...
    return (ldexp(((double)fixed[0]), -64) + ((double)((signed long)fixed[1])));
}

/* Die naive Umrechnung von mpx_t in double. */
#define mpx_get_d(fixed)                                                       \
    (ldexp(((double)(fixed)[1]), -64) + ldexp(((double)(fixed)[0]), -128))
...
#define mpx_add(summe, summand1, summand2)                                     \
    addf_ssaaaa(summe[1], summe[0], summand1[1], summand1[0], summand2[1],     \
                summand2[0])
...
#define mpxb_add_si(summe, summand1, summand2)                                 \
    addf_ssaaaa(summe[1], summe[0], summand1[1], summand1[0], summand2, 0)
